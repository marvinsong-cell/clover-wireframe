...
const OnboardingScreen = ({ onComplete }) => {
  ...
  const slides = [
    { icon: '🍀', title: '진짜 연결,\n클로버에서 시작하세요', description: '검증된 프로필로 신뢰할 수 있는\n만남을 제공합니다.' },
    { icon: '✨', title: '당신만의 클로버를\n완성하세요', description: '직업, 학력, 자산, 외모, 성격\n5가지 영역으로 나를 표현해요.' },
    { icon: '💚', title: '비슷한 사람과\n매칭되세요', description: '클로버 기반 매칭으로\n진지한 만남이 시작됩니다.' }
  ];
  ...
  return (
    <div className="screen">
      ...
      <CloverUI.BottomButton text={currentSlide < slides.length - 1 ? '다음' : '시작하기'} isActive={true} onClick={handleNext} />
    </div>
  );
};
